...
-d1)
    debut_temps=$(date +%s)
    awk -F';' '!seen[$6,$1]++ {print $6}' $fichier_csv | sort | uniq -c | sort -k1,1nr | head -n 10 > top10_conducteurs_trajets.txt
    fin_temps=$(date +%s)
...
-d2)
    debut_temps=$(date +%s)
    awk -F';' '{distances[$6]+=$5} END {for (i in distances) print distances[i], i}' $fichier_csv | sort -rn | head -n 10 > top10_conducteurs_distances.txt
    fin_temps=$(date +%s)
    duree=$((fin_temps - debut_temps))
...
-l)
    debut_temps=$(date +%s)
    awk -F';' '{distances[$1]+=$5} END {for (i in distances) print distances[i], i}' $fichier_csv | sort -k1,1nr | head -n 10 | sort -k2,2n > top10_trajets_distances.txt
    fin_temps=$(date +%s)
    duree=$((fin_temps - debut_temps))
